...
const FinancialListEnForm = () => {
  ...
  useEffect(() => {
    const newList = { boardId: 'EN_FIN', pageIndex: page, searchKeyword: null };
  ...
  // 검색
  const onSearch = (page) => {
    const newList = { boardId: 'EN_FIN', pageIndex: page, searchKeyword: searchKeyword };
    dispatch(selectBoard(newList));
  };
    ...
    <div className="a-content">
      ...
      <div className="ban-list p0">
        ...
        <div className="btn-area position">
          ...
          <Link to="/admin/investInfo/financialAddEn">
              ...
              {boardList.data?.map((list, index) => (
                <tr key={index}>
                  ...
                  <td className="tal pl40">
                    <Link to={`/admin/investInfo/financialInfoEn/${list.boardId}`}>{list.boardTitle}</Link>
                  </td>
                  <td>{list.createdDatetime}</td>
                  <td>
                    <Link to={`/admin/investInfo/financialModEn/${list.boardId}`}>
...
export default FinancialListEnForm;
