...
const OfficialNoticeListEnForm = () => {
  ...
  useEffect(() => {
    const newList = { boardId: 'EN_OFF', pageIndex: page, searchKeyword: null };
  ...
  const onEdit = (e, index, boardId, url) => {
    ...
    if (window.confirm('수정 하시겠습니까?')) {
      ...
      dispatch(updateBoard(newList)).then(() => {
        const newList = { boardId: 'EN_OFF', pageIndex: 1, searchKeyword: null };
        dispatch(selectBoard(newList));
  ...
  const onCreate = (e) => {
    ...
    if (window.confirm('등록 하시겠습니까?')) {
      const newList = { boardId: 'EN_OFF', boardTitle: boardTitle, url: url };
...
export default OfficialNoticeListEnForm;
